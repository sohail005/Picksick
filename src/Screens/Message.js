...
const Message = ({ navigation }) => {
  ...
  return (
    <View style={{ flex: 1, width: width, height: height, backgroundColor: backgroundColor }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', margin: 10, justifyContent: 'space-between' }}>
        <TouchableOpacity onPress={() => navigation.goBack()}>
          <Image source={require('../Assets/arrow.png')} style={{ width: 22, height: 22, tintColor: defaultTextColor }} resizeMode='contain' />
        </TouchableOpacity>
        <TouchableOpacity
          style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Text style={{ color: defaultTextColor, marginRight: 5, fontSize: 20 }}>Sohail.code</Text>
          <Image source={require('../Assets/arrow-down.png')} style={{ width: 16, height: 18, tintColor: defaultTextColor }} resizeMode='contain' />
        </TouchableOpacity>
      </View>
          ...
          return (
            <Animatable.View animation="zoomIn" duration={200} delay={index * 100} key={index}
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', }}>
                ...
                <View>
                  <Text style={{ color: defaultTextColor, marginLeft: 10, fontWeight: '700' }}>{item.accountName}</Text>
